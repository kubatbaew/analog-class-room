...
def works_students(request, pk):
    ...
    students_done_works = DoneWork.objects.select_related('student', 'work').filter(status="submitted")
    students_done_works_grade = DoneWork.objects.select_related('student', 'work').filter(status="done_grade")
...
def done_work_add_grade(request, pk):
    ...
    done_work.save()
    return redirect("detail_done_work", done_work.id)
